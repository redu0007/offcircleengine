package com.ditrans.offcirclev9engine;
...
public class ShakeRecognizer implements SensorEventListener {
    ...
    private static ShakeRecognizer mSensorEventListener;
    ...
    private int mThresholdInterval;
     ...
     * {@code listener} can be null. In that case, a IllegalArgumentException will be thrown.
     *
    ...
    public static boolean create(Context context, OnShakeListener listener) {
        ...
        mSensorEventListener = new ShakeRecognizer(listener);
     ...
     * should work for the majority of cases. It uses {@link ShakeRecognizer#DEFAULT_THRESHOLD_ACCELERATION}
     * for the sensibility,{@link ShakeRecognizer#DEFAULT_THRESHOLD_SHAKE_NUMBER} for the number of shake required and
     * {@link ShakeRecognizer#DEFAULT_INTERVAL} in ms to detect the next set of sensor data.
     *
     ...
     * @param interval    Interval to detect the next set of sensor data.
     */
    public static void updateConfiguration(float sensibility, int shakeNumber, int interval) {
        mSensorEventListener.setConfiguration(sensibility, shakeNumber, interval);
    }

    private ShakeRecognizer(OnShakeListener listener) {
        if (listener == null) {
        ...
        mThresholdInterval = DEFAULT_INTERVAL;
    }
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
        ...
        synchronized (mLock) {
            ...
            } else if (sensorBundle.getTimestamp() - mSensorBundles.get(mSensorBundles.size() - 1).getTimestamp() > mThresholdInterval) {
                mSensorBundles.add(sensorBundle);
            }
        }
    ...
    private void setConfiguration(float sensibility, int shakeNumber, int interval) {
        mThresholdAcceleration = sensibility;
        mThresholdShakeNumber = shakeNumber;
        mThresholdInterval = interval;
        synchronized (mLock) {
